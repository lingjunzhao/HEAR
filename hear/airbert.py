...
class Airbert(PreTrainedModel):
    ...
    def forward(
        ...
        # print("airbert token size: ", instr_tokens.size())
        (
            sequence_output_t,
            sequence_output_v,
            pooled_output_t,
        ...
        # print("ranking output size: ", outputs["ranking"].size())
...
class AirbertClassifier(Airbert):
    def __init__(self, config, dropout_prob=0.1):
        # super(AirbertClassifier, self).__init__(config, dropout_prob=dropout_prob)
        super().__init__(config)
        # self.model = Airbert.from_pretrained(
        #     from_pretrained, config, default_gpu=default_gpu
        # )
        self.classification_logit = torch.nn.Linear(config.bi_hidden_size, 1)

    def freeze_layers(self):
        model_parameters = filter(lambda p: p.requires_grad, self.parameters())
        print(f"number of trainable parameters: {sum(p.numel() for p in model_parameters):,}")

        for param in self.parameters():
            param.requires_grad = False
        for param in self.classification_logit.parameters():
            param.requires_grad = True
        # self.classification_logit.requires_grad = True
        trainable_model_parameters = filter(lambda p: p.requires_grad, self.parameters())
        print(f"number of trainable parameters after freezing: {sum(p.numel() for p in trainable_model_parameters):,}")
    ...
    def forward(
        ...
        # When using a DDP over multiple machines, PyTorch is complaining about unused outputs
        # outputs["ranking"] = self.model.vil_logit(pooled_output)
        outputs["classification_logits"] = self.classification_logit(pooled_output)
